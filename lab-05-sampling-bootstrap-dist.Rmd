...
> *Yes the bootstrap distribution relatively centered near the population mean, with the population mean at ~35.5, and the bootstrap mean ~35.9. Here the bootstrap sd represents the standard deviation (sd) of the bootstrap. In this case one sd of the bootstrap is 0.544 units.*
...
> *The bootstrap does look confident, it has a spread of ~2.5 with a high volume of counts near the mean. However this bootstrap is far off of the population mean. Precision is a measure of how well clumped a group of things are, in this case this bootstrap distribution is very precise. Accuracy is a measure of how close to the actual value or target something is. In this case this bootstrap is not very accurate. By using summer only data we are introducing systematic error.*
...
> *The summer only distribution has the highest precision, and lowest spread of data. However the random and stratified distributions both have much higher accuracy, although higher spread. The mean of the random distribution is slightly lower than the population mean and the mean of the stratified distribution is slightly higher. While both the random and stratified distributions appear fairly accurate, the stratified sample pulls from each month evenly. This may be desirable, however with each month having different number of days then some months may be disproportionately weighted in the distribution, especially February. For this reason I would be more inclined to choose the random distribution.*
...
quantile(boot_means_random, probs = c(0.975, 0.025))
```
...
*The confidence interval tells us that 95% of the samples exist above 34.77538, and below 36.93074. The confidence interval tells us about the uncertainty of the estimate through the range of the interval. A higher range indicates higher levels of uncertainty.*
---
...
*The red vertical lines represent the cutoff points for the 2.5 quartile, and the 97.5 quartile, respectively.*
---
...
boot_coef <- replicate(B, {
  ...
  samp_rows <- df_regression %>% slice_sample(n = nrow(df_regression), replace = TRUE)
...
*The slope confidence interval tells us that the true slope is likely between -0.4476754 and -0.3662247. The slope confidence interval does not include zero. It implies that the two are very likely corellated. If the slope confidence interval did include zero, then there would be a chance of 0 correlation.*
---
